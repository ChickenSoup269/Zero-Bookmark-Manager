export const en = {
  welcomeMessage: "Welcome to Zero Bookmark Manager Chat!",
  welcomeSubMessage: "How can I assist you today?",
  startButton: "Start Chat",
  allBookmarks: "All Bookmarks",
  sortDefault: "Sort: Default",
  sortNew: "New to Old",
  sortOld: "Old to New",
  sortLastOpened: "By Last Opened",
  sortAZ: "A to Z",
  sortZA: "Z to A",
  sortFavorites: "Favorites",
  sortHasNotes: "Has Notes",
  sortHasTags: "Has Tags",
  sortMostVisited: "Most Visited",
  sortDomain: "By Domain",
  createFolder: "Create Folder",
  addToFolder: "Add to Folder",
  deleteFolder: "Delete Folder",
  settings: "Settings",
  smartFoldersConfig: "Smart Folders:",
  smartFoldersShow: "Show",
  smartFoldersHide: "Hide",
  sidebarWidthConfig: "Sidebar Width:",
  tabDashboard: "Dashboard",
  tabQuickSave: "Quick Save",
  exportBookmarks: "Export Bookmarks",
  importBookmarks: "Import Bookmarks",
  selectAll: "Select All",
  showCheckboxes: "Show Checkboxes",
  hideCheckboxes: "Hide Checkboxes",
  searchPlaceholder: "Search bookmarks, tags, notes...",
  fontStyleTitle: "Font Style",
  viewModeTitle: "View Mode",
  viewFlat: "Flat List",
  viewTree: "Tree View",
  viewDetail: "Detail View",
  viewCard: "Card View",
  viewList: "List View",
  viewBento: "Bento Box",
  viewKanban: "Kanban Board",
  viewGallery: "Gallery View",
  renameTitle: "Rename",
  renameFolder: "Rename Folder",
  renamePlaceholder: "Enter new bookmark name",
  addToFolderTitle: "Add to Folder",
  moveToFolder: "Move to Folder",
  moveToFolderTitle: "Move Folder",
  selectFolder: "Select Folder",
  newFolderPlaceholder: "Enter folder name",
  createNewFolder: "Create",
  save: "Save",
  cancel: "Cancel",
  totalBookmarks: "Total Bookmarks",
  scrollToTop: "Scroll to Top",
  clear: "Clear",
  deleteConfirm: "Are you sure you want to delete this bookmark?",
  deleteFolderConfirm:
    "Are you sure you want to delete this folder? Bookmarks that exist in other folders will be preserved.",
  confirmDeleteTag:
    "Are you sure you want to delete this tag from the bookmark?",
  deleteTagSuccess: "Tag deleted successfully!",
  discardFolderPrompt: "You entered a folder name but didn't save. Discard it?",
  noBookmarks: "I couldn't find a bookmark with {0}.",
  emptyTitleError: "Please enter a title",
  emptyFolderError: "Please enter a folder name or select a folder",
  selectFolderError: "Please select a folder",
  errorUnexpected: "An unexpected error occurred",
  clearRenameAria: "Clear rename input",
  clearSearchAria: "Clear search input",
  settingsButtonAria: "Open settings menu",
  addToFolderOption: "Add to Folder",
  deleteBookmarkOption: "Delete",
  renameBookmarkOption: "Rename",
  editInNewTabOption: "Open Web View",
  openSidePanel: "Open Side Panel",
  deleteBookmarks: "Delete Selected",
  deleteSelected: "Delete Selected",
  deleteBookmarksConfirm:
    "Are you sure you want to delete the selected bookmarks?",
  deleteBookmarksSuccess: "Selected bookmarks deleted successfully!",
  move: "Move",
  edit: "Edit",
  delete: "Delete",
  noTags: "No tags",
  tagEmpty: "Tag name cannot be empty",
  downloadingModel: "Downloading model...",
  duplicateUrlError: "This URL is already bookmarked!",
  errorInvalidFolder: "Cannot delete this folder",
  errorImporting: "Error importing file. Check console.",
  searchResults: "Search Results",
  searchTitle: "Search",
  healthSortMalware: "Malware",
  configureShortcuts: "Configure Shortcuts",
  listView: "List",
  gridView: "Grid",
  treeView: "Tree",
  tags: "Tags",
  notes: "Notes",
  visits: "Visits",
  folderStudioTitle: "Folder Studio & Organizer",
  tabTreeStudio: "Hierarchy Studio",
  tabMergeDuplicates: "Merge Duplicates",
  tabAutoSort: "Auto Sort",
  tabCleanEmpty: "Clean Empty",
  searchFoldersPh: "Filter folders by name...",
  expandAll: "Expand All",
  collapseAll: "Collapse All",
  newSubfolder: "New Subfolder",
  folderDetails: "Folder Details",
  noFolderSelected: "Select a folder from the tree to view details",
  folderBookmarks: "Bookmarks in this folder",
  noBookmarksInFolder: "No bookmarks in this folder",
  safeDeleteFolder: "Delete Folder",
  safeDeletePrompt: "Choose how to delete this folder:",
  unnestBookmarksOption: "Delete folder only (move bookmarks to parent)",
  deleteAllOption: "Delete folder and all contents",
  mergeFoldersDesc:
    "Find and merge duplicate folders across branches into one single folder.",
  noDuplicateFoldersFound:
    "No duplicate folders found! Your folder structure is tidy.",
  duplicateGroupFound: "duplicate folders found with name",
  mergeIntoThis: "Merge into this folder",
  mergeSuccess: "Folders merged successfully!",
  autoSortDesc:
    "Sort folders and bookmarks in alphabetical order or by items count.",
  sortAlphabeticalAZ: "Sort A → Z (Alphabetical)",
  sortAlphabeticalZA: "Sort Z → A (Reverse)",
  sortByItemCount: "Sort by Item Count (Most items first)",
  sortScopeAll: "Apply to entire library",
  sortScopeSelected: "Apply to selected folder only",
  sortSuccess: "Folders sorted successfully!",
  emptyFoldersDesc:
    "Quickly find and remove all empty folders that contain no bookmarks or subfolders.",
  emptyFoldersFoundCount: "empty folders found.",
  cleanEmptySuccess: "Empty folders cleaned successfully!",
  cleanAllEmptyBtn: "Delete All Empty Folders",
  importSuccess: "Bookmarks imported successfully!",
  importDuplicatePrompt:
    "Some bookmarks already exist (same URL). Do you want to import the non-duplicate bookmarks?",
  importInvalidFile:
    "Invalid file format. Please select a valid JSON bookmark file.",
  importError: "Failed to import bookmarks. Please try again.",
  deleteBookmarkSuccess: "Bookmark deleted successfully!",
  exportPrompt:
    "Choose export format: JSON or HTML (data will be saved as JSON).",
  createFolderSuccess: "Folder created successfully!",
  confirmTitle: "Confirm",
  successTitle: "Success",
  errorTitle: "Error",
  renameSuccess: "Bookmark renamed successfully!",
  addToFolderSuccess:
    "Bookmark(s) added to folder successfully! (Reload to update data)",
  duplicateTitleError: "Title already exists in this folder",
  noBookmarksSelected: "No bookmarks selected",
  restartExtension: "Please try again or restart the extension.",
  exportTitle: "Export Bookmarks",
  advancedSettings: "Advanced Settings (HTML & CSV)",
  includeCreationDates: "Include creation dates",
  includeFolderModDates: "Include folder modification dates",
  popupSubtitle: "Choose your preferred export format and settings",
  exportFormat: "Export Format",
  exportPresets: "Export Presets",
  exportPresetBackup: "Full Backup",
  exportPresetBackupText: "JSON restore file",
  exportPresetWeb: "Web Archive",
  exportPresetWebText: "Interactive HTML",
  exportPresetSelected: "Share Selected",
  exportPresetSelectedText: "CSV selected items",
  exportPresetBrowser: "Browser Import",
  exportPresetBrowserText: "Netscape HTML",
  jsonDescription: "Machine readable format (backup and restore with import)",
  htmlDescription: "Interactive web page",
  csvDescription: "Tabular data (easily open in Excel/Sheets)",
  netscapeDescription: "Standard browser format",
  includeCreationDatesDescription: "Add creation timestamps",
  includeFolderModDatesDescription: "Add folder timestamps",
  exporting: "Exporting...",
  bookmarksTitle: "My Bookmarks Collection",
  bookmarksHeader: "My Bookmarks",
  exportedOn: "Exported on",
  loadingBookmarks: "Loading bookmarks...",
  generatedBy: "Generated by Zero Bookmark Manager Extension",
  bookmarks: "bookmarks",
  folders: "folders",
  created: "Created",
  modified: "Modified",
  unnamedFolder: "Unnamed Folder",
  notAvailable: "N/A",
  aiTitle: "Configure AI Chatbot",
  aiConfigTitle: "AI Settings",
  aiProvider: "AI Provider",
  apiKeyLabel: "API Key",
  getApiKey: "Get API Key",
  modelLabel: "Model Name",
  aiLocalOption: "Chrome Built-in AI (Experimental)",
  localAiExperimentalNote:
    "Note: This is an experimental Chrome feature and may not be available in all browsers or regions.",
  localOnlyOption: "Offline Mode (No AI)",
  localOnlyDescription: "No AI provider needed. Supports basic commands only.",
  localOnlyNotSupported:
    "This command is not supported in Offline Mode. Please enable an AI provider for advanced tasks.",
  localGeminiNotAvailable:
    "Chrome Built-in AI is not available. Please enable it in chrome://flags.",
  localAiGuideSteps:
    "<div style='text-align:left; line-height:1.6; font-size: 0.95em;'><b>How to enable Chrome Built-in AI:</b><br><br>1. Copy & paste into address bar: <br><code style='background:var(--bg-tertiary);padding:3px 6px;border-radius:4px;user-select:all;'>chrome://flags/#prompt-api</code><br>👉 Set to <b>Enabled</b> (or Enabled Multilingual)<br><br>2. Copy & paste: <br><code style='background:var(--bg-tertiary);padding:3px 6px;border-radius:4px;user-select:all;'>chrome://flags/#prompt-api-multimodal-input</code><br>👉 Set to <b>Enabled</b><br><br>3. Click <b>Relaunch</b> at the bottom right.<br><br><i>Note: On first run, Chrome will download the AI model (~1.5GB). You will see a progress bar in the chat.</i></div>",
  learnHowToEnable: "Learn how to enable",
  localAiGuideTitle: "How to Enable Chrome Built-in AI",
  copyToClipboard: "Copy to Clipboard",
  copySuccess: "Copied!",
  aiConfigSaveSuccess: "AI settings saved!",
  favourite: "Favourite",
  removeFavourite: "Remove from Favorites",
  favoriteSuccess: "Bookmark added to favorites!",
  unfavoriteSuccess: "Bookmark removed from favorites!",
  helpGuideTitle: "Help Guide",
  helpGuideSearchPlaceholder: "Search commands or features...",
  helpGuideSearchAria: "Search help guide",
  helpGuideClearSearch: "Clear search",
  helpGuideNoResults: "No matching commands or features found.",
  helpGuideUseCommand: "Click to send this command to chat",
  copyCommand: "Copy command",
  aiStatusReady: "AI Ready",
  aiStatusSetup: "Setup AI",
  aiStatusLocal: "Local AI",
  aiStatusBasic: "Basic",
  commandPaletteTitle: "Command Palette",
  commandPalettePlaceholder: "Search bookmarks, notes, or run command...",
  commandPaletteAria: "Command palette search",
  commandPaletteOpen: "Open command palette",
  commandPaletteNoResults: "No matching command or bookmark.",
  commandPaletteView: "View",
  commandPaletteViewMeta: "Change bookmark layout",
  commandPaletteTheme: "Theme",
  commandPaletteThemeMeta: "Switch appearance",
  commandPaletteFocusSearch: "Focus bookmark search",
  commandPaletteFocusSearchMeta: "Jump to sidebar search",
  commandPaletteClearSearch: "Clear search and filters",
  commandPaletteClearSearchMeta: "Show all bookmarks again",
  commandPaletteCreateFolderMeta: "Open folder creator",
  commandPaletteExportMeta: "Open export options",
  commandPaletteImportMeta: "Import a backup file",
  commandPaletteCheckLinksMeta: "Scan bookmark health",
  commandPaletteAiMeta: "Configure chat provider",
  commandPaletteHelpMeta: "Open command guide",
  commandPaletteSelectionMeta: "Toggle bulk selection",
  commandPaletteCopyUrl: "Copy URL",
  commandPaletteOpenUrl: "Open URL",
  commandPaletteFilter: "Filter",
  commandPaletteFolderMeta: "Filter by folder",
  commandPaletteReadingQueue: "Reading Queue",
  commandPaletteReadingQueueMeta: "Show saved read-later bookmarks",
  sidebarWorkspaces: "Workspaces",
  workspaceWork: "Work",
  workspaceLearning: "Learning",
  workspaceDesign: "Design",
  workspacePersonal: "Personal",
  workspaceSaveCurrent: "Save current view",
  workspaceSaved: "Workspace saved.",
  workspaceApplied: "Workspace applied.",
  workspaceEmpty: "No saved view",
  workspaceSavedView: "Saved view",
  workspaceDeleteSaved: "Delete saved view",
  workspaceDeleted: "Workspace deleted.",
  confirmDeleteWorkspace: "Delete this saved workspace?",
  noAvailableTags: "No available tags",
  undoAction: "Undo",
  undoSuccess: "Action undone.",
  undoFailed: "Could not undo this action.",
  undoRenameMessage: "Bookmark renamed.",
  undoDeleteMessage: "Bookmark deleted.",
  undoBulkDeleteMessage: "Bookmarks deleted.",
  smartCleanupTitle: "Smart Cleanup",
  smartCleanupShort: "Cleanup",
  smartCleanupSubtitle:
    "Review bookmark cleanup opportunities before changing anything.",
  smartCleanupDuplicates: "Duplicates",
  smartCleanupDeadLinks: "Dead links",
  smartCleanupStale: "Old unused",
  smartCleanupUntagged: "Untagged",
  smartCleanupEmptyFolders: "Empty folders",
  smartCleanupRemoveDuplicates: "Review remove",
  smartCleanupScanLinks: "Scan links",
  smartCleanupFilter: "Filter",
  smartCleanupReview: "Review",
  smartCleanupDeleteEmpty: "Delete Empty Folders",
  smartCleanupDeleteStale: "Delete Old Unused",
  smartCleanupDeleteDead: "Delete Dead Links",
  smartCleanupFilterUntagged: "Filter Untagged",
  smartCleanupConfirmDelete:
    "Are you sure you want to delete these items? This cannot be undone.",
  smartCleanupNone: "Nothing to review here.",
  smartCleanupMore: "more",
  smartCleanupTotalIssues: "items to review",
  smartCleanupDomains: "top domains",
  smartCleanupGroupName: "Enter folder name for grouped domains:",
  smartCleanupNoDomains: "No domains with multiple bookmarks found.",
  smartCleanupGroupSuccess:
    "Successfully grouped {count} bookmarks into {folder}",
  smartCleanupGroupError: "Error organizing bookmarks.",
  smartCleanupTagConfirm: "Auto-tag all bookmarks based on their domains?",
  smartCleanupTagSuccess: "Added {count} new domain tags!",
  smartCleanupTagNone: "No new tags needed (or tag limits reached).",
  guideTitle: "How to use:",
  smartCleanupGroupDomain: "Group by Domain",
  guideGroupDomain:
    "Creates folders for domains with multiple bookmarks and moves them inside.",
  smartCleanupAutoTag: "Auto-Tag Domains",
  guideAutoTag:
    "Extracts the domain name and adds it as a tag to each bookmark.",
  smartCleanupNotes: "With notes",
  smartCleanupDuplicateIntro:
    "These are extra copies. The best titled/newest item is kept by the cleanup action.",
  smartCleanupDeadIntro: "Run a fresh scan first if these results look old.",
  smartCleanupStaleIntro: "Older than one year with no tracked visits.",
  smartCleanupUntaggedIntro:
    "Good candidates for the new tag suggestions in Bookmark Detail.",
  smartCleanupNextTitle: "Next up",
  smartCleanupNextWorkspaces: "Pinned workspaces",
  smartCleanupNextWorkspacesText:
    "Save views for Work, Learning, Design, and Personal.",
  smartCleanupNextNotesSearch: "Notes search",
  smartCleanupNextNotesSearchText:
    "Search bookmark notes from sidebar and Command Palette.",
  smartCleanupNextPresets: "Export presets",
  smartCleanupNextPresetsText:
    "One-click Backup, Share Selected, CSV, and Netscape exports.",
  smartCleanupGuideTitle: "Usage Guide",
  smartCleanupGuideStart: "Quick Start",
  smartCleanupGuideStartText:
    "Click any card on the left to review items and take targeted cleanup actions.",
  smartCleanupGuideMerge: "Merge Duplicates",
  smartCleanupGuideMergeText:
    "Combine identical URLs safely. The most recently updated bookmark is always kept.",
  smartCleanupGuideDead: "Dead Links",
  smartCleanupGuideDeadText:
    "Scan and identify bookmarks that are no longer accessible (404, server errors).",
  smartCleanupGuideOrganize: "Organize Domains",
  smartCleanupGuideOrganizeText:
    "Group your most visited domains into organized folders or auto-apply tags.",
  duplicateMergeTitle: "Duplicate Merge",
  duplicateMergeSubtitle:
    "Choose the bookmark to keep in each duplicate group. Metadata from removed copies will be merged into it.",
  duplicateMergeApply: "Merge selected",
  duplicateMergeGroups: "duplicate groups",
  duplicateMergeWillRemove: "copies to remove",
  duplicateMergeMetadata: "Tags, notes, pins",
  duplicateMergeMetadataHint: "will be preserved",
  duplicateMergeFailed: "Duplicate merge failed.",
  merging: "Merging...",
  refresh: "Refresh",
  fullscreen: "Fullscreen",
  exitFullscreen: "Exit Fullscreen",
  bookmarkNotesPlaceholder: "Add private notes for this bookmark...",
  bookmarkNotesSaved: "Notes saved.",
  readLater: "Read Later",
  readingQueued: "In Reading Queue",
  suggestTags: "Suggest Tags",
  autoTagSuggestionsTitle: "Suggested tags",
  saveNotes: "Save Notes",
  noTagSuggestions: "No new tag suggestions.",
  tagAddedSuccess: "Tag added.",
  exportChatHistory: "Export Chat History",
  maximizeMinimize: "Maximize/Minimize",
  editAIConfig: "AI Settings",
  closeChat: "Close Chat",
  chatToggle: "Open Chat",
  reportBug: "Report Bug",
  reportBugUrl:
    "https://docs.google.com/forms/d/e/1FAIpQLScK_33RwIOSBcyqiLc28kRl-kDxrb5R-XBn3J1i_ckXSUgzfQ/viewform?usp=dialog",
  reportBugConfirm:
    "You are about to be redirected to the bug report form. Please include the following information in your report to help us fix the issue faster:",
  browserInfo: "Browser Info",
  extensionVersion: "Extension Version",
  continueToForm: "Continue to Form",
  noChatHistory: "No chat history available",
  loadingChat: "Processing...",
  moveBookmarkSuccess: "Bookmark moved successfully",
  moveBookmarkError: "Bookmark not found or invalid folder",
  duplicateFolderError: "A folder with this name already exists or is invalid.",
  renameError: "Failed to rename folder. Please try again.",
  suggestWebsite: "I've suggested the following websites",
  searchFolderResult: "Found folders",
  noFoldersFound: "No folders found for",
  aiBookmarkResponse: "AI Response",
  notSupported: "Sorry, I can only help with bookmark-related tasks",
  youHave: "You have",
  hereAreYourBookmarks: "Here are your bookmarks",
  hereAreYourFolders: "Here are your folders",
  hereAreBookmarksInFolder: "Here are the bookmarks in folder",
  noBookmarksInFolder: "No bookmarks in this folder",
  addedBookmarkToFolder: "I've added the bookmark",
  toFolder: "to the folder",
  updatedBookmark: "I've updated the bookmark",
  inFolder: "in",
  deletedBookmark: "I've deleted the bookmark",
  foundBookmarks: "I found",
  bookmarksMatching: "bookmarks matching",
  noBookmarksFoundFor: "I couldn't find any bookmarks matching",
  foundFolders: "I found these folders",
  noFoldersFoundFor: "I couldn't find any folders matching",
  movedBookmark: "I've moved the bookmark",
  noFolders: "You don't have any folders yet.",
  createdFolder: "I have created the folder",
  renamedFolder: "I have renamed the folder",
  to: "to",
  alreadyExists: "already exists.",
  andAllItsContents: "and all its contents",
  deletedFolder: "I have deleted the folder",
  deleteFolderCancelled: "Folder deletion cancelled",
  viewChanged: "View changed to",
  invalidView: "Invalid view mode",
  themeChanged: "Theme changed to",
  invalidTheme: "Invalid theme",
  sortChanged: "Sort order changed to",
  invalidSort: "Invalid sort type",
  checkingLinks:
    "Started checking for broken links. Results will appear in the UI.",
  customLangInstruction1:
    "1. Click 'Copy Prompt & Template' to copy the instructions for AI.",
  customLangInstruction2:
    "2. Paste it into ChatGPT or Gemini AI Studio to translate.",
  customLangInstruction3:
    "3. Copy the returned JSON code and paste it back into the textbox below.",
  copyPromptAndTemplate: "Copy Prompt & Template",
  importJsonFile: "Import JSON",
  insertLanguageTemplate: "Insert Template",
  helpCommands:
    '\n      <div class="chat-help-list">\n        <p><strong>Available Commands:</strong></p>\n        <p>• <code>list bookmarks</code> / <code>count</code></p>\n        <p>• <code>search bookmark "keyword"</code></p>\n        <p>• <code>create folder "Name"</code></p>\n        <p>• <code>rename folder "Old" to "New"</code></p>\n        <p>• <code>move "Title" to @Folder</code></p>\n        <p>• <code>favorite "Title"</code> / <code>delete id [ID]</code></p>\n        <p>• <code>view [card/list/tree/detail]</code></p>\n        <p>• <code>theme [dark/dracula/light/...]</code></p>\n        <p>• <code>sort [a-z/newest/domain/...]</code></p>\n        <p>• <code>check links</code> / <code>check duplicates</code></p>\n        <p>• <code>suggest website about [topic]</code></p>\n        <p>• <code>generate qr</code> / <code>export chat</code></p>\n        <p style="margin-top:8px; opacity:0.8; font-size:0.8rem"><em>Click the (?) icon for full visual guide with cards.</em></p>\n      </div>\n    ',
  helpGuide:
    '\n  <div class="guide-container v2">\n    <!-- Command List Section (Visual Cards) -->\n    <div class="guide-header-v2">\n      <h3><i class="fas fa-terminal"></i> Command Explorer</h3>\n      <p>Quick reference for all supported chat commands.</p>\n    </div>\n\n    <div class="command-list-container">\n        <div class="command-section">\n          <h5><i class="fas fa-magic"></i> AI & SMART</h5>\n          <div class="cmd-row"><code>suggest website [topic]</code> <span>AI Discovery</span></div>\n          <div class="cmd-row"><code>add [url]</code> <span>Auto organize</span></div>\n          <div class="cmd-row"><code>check links</code> <span>Health check</span></div>\n          <div class="cmd-row"><code>check duplicates</code> <span>Mass cleanup</span></div>\n        </div>\n\n        <div class="command-section">\n          <h5><i class="fas fa-bookmark"></i> BOOKMARKS</h5>\n          <div class="cmd-row"><code>list bookmarks</code> <span>Show all</span></div>\n          <div class="cmd-row"><code>search bookmark "key"</code> <span>Find items</span></div>\n          <div class="cmd-row"><code>favorite "Title"</code> <span>Pin/Fav</span></div>\n          <div class="cmd-row"><code>edit bookmark "Name"</code> <span>Update data</span></div>\n          <div class="cmd-row"><code>delete id [number]</code> <span>Exact remove</span></div>\n          <div class="cmd-row"><code>visit counts</code> <span>Usage stats</span></div>\n        </div>\n\n        <div class="command-section">\n          <h5><i class="fas fa-folder"></i> FOLDERS</h5>\n          <div class="cmd-row"><code>list folders</code> <span>Tree view</span></div>\n          <div class="cmd-row"><code>in "Folder"</code> <span>View contents</span></div>\n          <div class="cmd-row"><code>create folder "Name"</code> <span>New folder</span></div>\n          <div class="cmd-row"><code>rename folder "A" to "B"</code> <span>Rename</span></div>\n          <div class="cmd-row"><code>delete folder "Name"</code> <span>Wipe contents</span></div>\n        </div>\n\n        <div class="command-section">\n          <h5><i class="fas fa-palette"></i> UI & SETTINGS</h5>\n          <div class="cmd-row">\n            <code>view [mode]</code>\n            <span class="opt-pills">card, list, tree, detail</span>\n          </div>\n          <div class="cmd-row">\n            <code>theme [style]</code>\n            <span class="opt-pills">dark, light, dracula, onedark, tokyonight, monokai, tet, github-blue, winter-is-coming, nord, synthwave, gruvbox, catppuccin, nightowl, nord-light, gruvbox-light, catppuccin-light, nightowl-light</span>\n          </div>\n          <div class="cmd-row">\n            <code>sort [mode]</code>\n            <span class="opt-pills">a-z, z-a, newest, oldest, most-visited, last-opened, domain</span>\n          </div>\n          <div class="cmd-row">\n            <code>font [family]</code>\n            <span class="opt-pills">gohu, anonymice, jetbrains, profont</span>\n          </div>\n        </div>\n    </div>\n\n    <div class="custom-language-guide">\n      <div class="guide-header-v2">\n        <h3><i class="fas fa-language"></i> Custom Language JSON</h3>\n        <p>Create your own translation pack with AI, then copy the JSON or save it as a .json file.</p>\n      </div>\n\n      <div class="language-guide-grid">\n        <div class="language-guide-step">\n          <span>1</span>\n          <strong>Ask AI</strong>\n          <p>Open ChatGPT or Gemini AI Studio and paste the prompt below.</p>\n        </div>\n        <div class="language-guide-step">\n          <span>2</span>\n          <strong>Copy JSON</strong>\n          <p>Ask the AI to return only valid JSON with no Markdown fences.</p>\n        </div>\n        <div class="language-guide-step">\n          <span>3</span>\n          <strong>Use or Export</strong>\n          <p>Copy the JSON into your custom language flow, or save it as <code>my-language.json</code>.</p>\n        </div>\n      </div>\n\n      <div class="language-ai-links">\n        <a href="https://chatgpt.com/" target="_blank" rel="noopener noreferrer"><i class="fas fa-comments"></i> Open ChatGPT</a>\n        <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer"><i class="fas fa-robot"></i> Open Gemini AI Studio</a>\n      </div>\n\n      <div class="command-section language-prompt-box">\n        <h5><i class="fas fa-wand-magic-sparkles"></i> Prompt to copy</h5>\n        <code>You are creating a custom language JSON for Zero Bookmark Manager. Translate every value into [YOUR LANGUAGE]. Keep all keys unchanged. Preserve placeholders like {0}, {title}, HTML tags, URLs, command words inside code examples, and product names. Return only valid JSON, no Markdown. Use this shape:\n{\n  "languageCode": "your-locale-code",\n  "languageName": "Your Language Name",\n  "translations": {\n    "allBookmarks": "All Bookmarks",\n    "settings": "Settings",\n    "quickOpenTitle": "Quick Open Action",\n    "quickOpenPopup": "Default Popup",\n    "quickOpenSidePanel": "Default Side Panel",\n    "quickOpenWeb": "Default Web Tab"\n  }\n}\nIf I provide the full English translations object, translate all values and return the complete JSON file.</code>\n      </div>\n    </div>\n\n    <!-- Feature Pillars Section -->\n    <div class="guide-header-v2" style="margin-top:25px">\n      <h3><i class="fas fa-book-open"></i> Feature Guide</h3>\n      <p>Master your library with these 5 core pillars.</p>\n    </div>\n\n    <div class="guide-section-v2">\n      <div class="guide-card-v2 main">\n        <div class="card-icon"><i class="fas fa-terminal"></i></div>\n        <div class="card-content">\n          <h4>1. Interactive Control</h4>\n          <p>Talk to the AI to manage everything. Type <code>list</code> to see your data or <code>card</code> to change layout.</p>\n        </div>\n      </div>\n\n      <div class="guide-card-v2 main">\n        <div class="card-icon"><i class="fas fa-tags"></i></div>\n        <div class="card-content">\n          <h4>2. Tagging & Pinning</h4>\n          <p>Add up to 10 custom colored tags per bookmark. Use <code>favorite "My Site"</code> to keep important links at the top.</p>\n        </div>\n      </div>\n\n      <div class="guide-card-v2 ai">\n        <div class="card-icon"><i class="fas fa-shield-alt"></i></div>\n        <div class="card-content">\n          <h4>3. System Health</h4>\n          <p>Keep your library clean. Use <code>check links</code> to find dead URLs and <code>check duplicates</code> to wipe clones.</p>\n        </div>\n      </div>\n\n      <div class="guide-card-v2 ai">\n        <div class="card-icon"><i class="fas fa-brain"></i></div>\n        <div class="card-content">\n          <h4>4. AI Discovery</h4>\n          <p>Configure Gemini/GPT to unlock: <code>suggest website about [topic]</code>, Auto-categorization, and Smart Search.</p>\n        </div>\n      </div>\n\n      <div class="guide-card-v2 main">\n        <div class="card-icon"><i class="fas fa-file-import"></i></div>\n        <div class="card-content">\n          <h4>5. Data Portability</h4>\n          <p>Export to <b>JSON</b>, <b>HTML</b>, <b>CSV</b>, or <b>Netscape</b>. Import instantly from any JSON backup.</p>\n        </div>\n      </div>\n\n      <div class="guide-card-v2 main">\n        <div class="card-icon"><i class="fas fa-desktop"></i></div>\n        <div class="card-content">\n          <h4>6. Webview Optimization</h4>\n          <p>Open full-screen manager. <b>Drag and drop bookmarks into folders on the sidebar</b> easily, deeply customize UI and advanced settings.</p>\n        </div>\n      </div>\n    </div>\n\n    <div class="experimental-badge">\n      <i class="fas fa-lightbulb"></i> Shortcut: Type <code>@</code> or <strong>ID</strong> in any command for instant selection.\n    </div>\n  </div>\n',
  clarifyBookmark:
    "I found multiple bookmarks named '{title}'. Please provide the URL or folder to specify which one.",
  naturalLanguagePrompt:
    "I don't know. Try commands like 'delete bookmark ID 123' or 'rename bookmark ID 456 to NewTitle'.",
  markedFavorite: "I've marked the bookmark",
  unmarkedFavorite: "I've removed the bookmark from favorites",
  showBookmarkIds: "Show Bookmark IDs",
  hideBookmarkIds: "Hide Bookmark IDs",
  showFolderListBg: "Show List Background",
  hideFolderListBg: "Hide List Background",
  localStorageSettingsTitle: "Local Storage Settings",
  restartGuideTitle: "Show Guide Again",
  restartGuideStarted: "Guide restarted.",
  saveSearchQuery: "Save Search Query",
  saveSelectedFolderId: "Save Selected Folder",
  saveSortType: "Save Sort Type",
  saveViewMode: "Save View Mode",
  saveCollapsedFolders: "Save Collapsed Folders",
  saveCheckboxesVisible: "Save Checkboxes Visibility",
  allTags: "All Tags",
  openTagsBrowser: "All Tags",
  searchTagsPlaceholder: "Search tags...",
  clearTagFilter: "Clear tag filter",
  noTagsFound: "No tags found",
  viewDetail: "View Details",
  manageTags: "Manage Tags",
  selectTag: "Select Tag",
  tagExists: "Tag already exists",
  tagLimitError: "Cannot add more than 10 tags per bookmark",
  saveSelectedTag: "Save Selected Tags",
  addTag: "Add Tag",
  newTagPlaceholder: "Enter tag name...",
  currentTagsLabel: "Current Tags:",
  addNewTagLabel: "Add New Tag:",
  tagBgColorLabel: "BG",
  tagTextColorLabel: "Text",
  addFromExistingLabel: "Or add from existing:",
  doneLabel: "Done",
  tagLimitReached: "Tag limit reached",
  viewListView: "List View",
  detailDateAdded: "Date Added",
  detailFolder: "Folder",
  includeFolderPath: "Include Folder Path (for CSV)",
  includeFolderPathDescription: "Add full folder path (for CSV)",
  includeIconData: "Include icon data (Base64)",
  includeIconDataDescription: "Add favicon data to bookmarks",
  exportOnlySelected: "Only export selected bookmarks",
  exportOnlySelectedDescription:
    "If any bookmarks are checked in the main list, export only those",
  pinSuccess: "Bookmark pinned!",
  unpinSuccess: "Bookmark unpinned!",
  pinToTop: "Pin to Top",
  unpin: "Unpin from Top",
  errorCannotMoveFolderToSelf: "Cannot move folder to itself.",
  errorCannotMoveFolderToDescendant: "Cannot move folder to its descendant.",
  organizeFolders: "Organize Folders",
  organizeFoldersTitle: "Organize Folders",
  moveFolderSuccess: "Folder moved successfully!",
  filtersTitle: "Filters",
  cloudSync: "Cloud Sync",
  searchOptions: "Search options",
  folderActionsTitle: "Folder actions",
  selectionTitle: "Selection",
  healthSortAll: "Sort by Health: All",
  healthSortDead: "Dead Links",
  healthSortSuspicious: "Suspicious Links",
  healthSortSafe: "Safe Links",
  generateQrCode: "Generate QR",
  noDuplicatesFound: "No duplicate bookmarks found.",
  duplicatesRemoved: "duplicate(s) removed.",
  dupScopeTitle: "Duplicate Scope:",
  dupScopeFolder: "Only 1 folder",
  dupScopeAll: "Across folders",
  dupConfirmMsgFolder:
    "Found {0} duplicate bookmarks in the same folders. Do you want to remove them?",
  dupConfirmMsgAll:
    "Found {0} duplicate bookmarks across all folders. Do you want to remove them?",
  autoRemoveDupTitle: "Auto Remove Duplicates (Background)",
  checkLinks: "Check Links",
  checkDuplicates: "Check Duplicates",
  bookmarkActionsTitle: "Bookmark Actions",
  appSettingsTitle: "Application Settings",
  extensionOptionsTitle: "Extension Options",
  themeSelectionTitle: "Themes",
  bookmarkHealthTitle: "Bookmark Health & Duplicates",
  bookmarkHealthSmallTitle: "Check Bookmarks",
  autoAiCategorizeTitle: "Auto Categorize with AI (Background)",
  quickOpenTitle: "Quick Open Action",
  quickOpenQuickSave: "Quick Save",
  quickOpenPopup: "Default Popup",
  quickOpenSidePanel: "Default Side Panel",
  quickOpenWeb: "Full Page",
  quickOpenNativePopup: "Browser Popup",
  showNotesPreview: "Show Notes Preview",
  showTagsInView: "Show Tags in View",
  showFolderCount: "Show Bookmark Count in Folders",
  folderCountMode: "Folder Item Badge",
  countModeBookmarks: "Bookmarks",
  countModeFolders: "Folders",
  countModeBoth: "Both",
  countModeOff: "Off",
  buttonWeightTitle: "Button weight",
  buttonWeightAll: "All",
  buttonWeightBookmarks: "Bookmarks",
  buttonWeightSidebar: "Sidebar",
  buttonWeightSettings: "Settings",
  buttonWeightNormal: "Normal",
  buttonWeightBold: "Bold",

  checkHealthModalTitle: "Check Link Health",
  checkHealthModalDesc:
    "Choose scope to check for dead (404/500), malware, or risky links:",
  checkDuplicatesModalTitle: "Check Duplicate Bookmarks",
  checkDuplicatesModalDesc:
    "Choose scope to check for duplicate bookmark URLs:",
  scopeAllBookmarks: "All Bookmarks",
  scopeAllBookmarksDesc: "Scan all bookmarks across the entire browser",
  scopeByFolder: "By Specific Folder",
  scopeByFolderDesc: "Scan only bookmarks within the selected folder",
  scopeIncludeSubfolders: "Include subfolders",
  scopeIncludeSubfoldersHint: "Also scan bookmarks in all nested subfolders",
  scopeSearchFolderPlaceholder: "Search folders...",
  scopeQuickSelect: "Quick:",
  scopeCurrentFolder: "Current Folder",
  scopeNoFoldersFound: "No matching folders found",
  scopeEstimatedTime: "Estimated time: ~{0}s",
  scopePreviewCheckLinks: "Will check: {0} links",
  scopePreviewCheckDuplicates: "Will scan: {0} bookmarks",
  scopeNoBookmarks: "No bookmarks found in the selected scope.",
  scopeStartCheck: "Start Check",
  checkLinksInFolder: "Check Links in Folder",
  checkDuplicatesInFolder: "Check Duplicates in Folder",
  checkHealthProgressCount: "Checking {0} links... Please wait.",
  checkHealthCompletedIssues:
    "Finished! Checked {0} links. Found {1} broken or suspicious links.",
  checkHealthCompletedClean:
    "Finished! Checked {0} links. All appear healthy and safe.",

  addCustomLanguage: "Add custom language",
  customLanguageTitle: "Add Custom Language",
  customLanguageHint:
    "Paste JSON from ChatGPT or Gemini AI Studio, or import a .json language file.",
  customLanguagePlaceholder:
    '{"languageCode":"es","languageName":"Español","translations":{"settings":"Ajustes"}}',
  customLanguagePresetTitle: "Start with a language",
  customLanguagePresetHint:
    "These presets only fill the JSON name/code. Paste the template into AI to translate the values.",
  saveCustomLanguage: "Save Language",
  customLanguageSaved: "Custom language saved!",
  customLanguageInvalid:
    "Invalid language JSON. Use languageCode, languageName, and translations.",
  customLanguageTemplateName: "My Language",
  sidebarAllBookmarks: "All Bookmarks",
  sidebarFavorites: "Favorites",
  sidebarUnsorted: "Unsorted",
  sidebarRecent: "Recently Added (7 days)",
  sidebarMostVisited: "Most Visited",
  sidebarUntagged: "Untagged",
  smartFolderResults: "Smart Folder",
  sidebarFolders: "Folders",
  sidebarTags: "Tags",
  sidebarSort: "Sort",
  sidebarManage: "Manage",
  sidebarMoreFolders: "more folders",
  sidebarBookmarks: "bookmarks",
  sidebarListBg: "BG Color Bookmarks",
  aiProfileQuickSwitch: "Quick Switch Profile",
  aiProfileSaveNew: "Save New Profile",
  aiProfileDeleteSelected: "Delete Selected Profile",
  aiProfileNamingTitle: "Name your Profile",
  aiProfileNamePlaceholder: "e.g., Gemini Free 1...",
  aiProfileApplySuccess: "Applied profile:",
  aiProfileSelectToDelete: "Please select a profile to delete",
  aiProfileDeleteConfirm: 'Are you sure you want to delete profile "{0}"?',
  aiProfileDeleteSuccess: "Deleted profile:",
  aiProfileNamePrompt: "Enter a name for this profile:",
  aiProfileSaveSuccess: "Profile saved successfully!",
  aiProfileNoProfiles: "No profiles saved yet",
  firstRunLanguageTitle: "Choose your language",
  firstRunLanguageSub: "Select the language you want to use first.",
  firstRunTitle: "Welcome to Zero Bookmark",
  firstRunSub: "Add bookmarks like Chrome, then choose your default font:",
  firstRunGuideStep1:
    "Open any website and click the Chrome star in the address bar.",
  firstRunGuideStepShortcut:
    "Use Ctrl + D on Windows/Linux or Command + D on macOS to bookmark the current page faster.",
  firstRunGuideStep2: "Choose a folder, then press Done to save the bookmark.",
  firstRunGuideStep3: "Open Zero Bookmark to manage, search, and organize it.",
  firstRunGuideStepOrganize:
    "Folder Studio: Visual 2-column workspace with drag & drop, duplicate merge, auto sort, and clean empty folders.",
  firstRunGuideStepAnalytics:
    "Analytics & Insights: Visual metrics on your bookmark trends, habits, health ratio, and smart recommendations.",
  firstRunSave: "Get Started",
  firstRunTourSettingsTitle: "Open Settings",
  firstRunTourSettingsMsg:
    "Use this button to change language, theme, font, view mode, export/import, and app behavior.",
  firstRunTourPanelTitle: "Settings Panel",
  firstRunTourPanelMsg:
    "Everything is grouped here: extension options, bookmark actions, health checks, and application settings.",
  firstRunTourWebTitle: "Open Web View",
  firstRunTourWebMsg:
    "Choose this when you want the full bookmark manager page with more room for organizing.",
  firstRunTourSidePanelTitle: "Open Side Panel",
  firstRunTourSidePanelMsg:
    "Use the side panel when you want bookmarks beside the current tab while browsing.",
  firstRunTourSearchTitle: "Search And Filter",
  firstRunTourSearchMsg:
    "Search bookmarks by title, URL, tag, or note. Combine it with folders, sort modes, and views.",
  firstRunTourBookmarkMenuTitle: "Bookmark Options",
  firstRunTourBookmarkMenuMsg:
    "Use the three-dot button on each bookmark to pin, move to folder, rename, delete, view details, manage tags, generate QR, or mark as favorite.",
  firstRunWebTourSidebarTitle: "Main Sidebar",
  firstRunWebTourSidebarMsg:
    "This button opens or collapses the left sidebar. The web view is built around this sidebar for faster organizing.",
  firstRunWebTourCommandTitle: "Command Palette",
  firstRunWebTourCommandMsg:
    "Open quick commands for search, view changes, cleanup, navigation, and common bookmark actions.",
  firstRunWebTourWorkspacesTitle: "Workspaces",
  firstRunWebTourWorkspacesMsg:
    "Save a current view as Work, Learning, Design, or Personal so you can return to it quickly.",
  firstRunWebTourFoldersTitle: "Folders",
  firstRunWebTourFoldersMsg:
    "Browse the Chrome bookmark tree, create folders. Notably, you can easily drag and drop bookmarks directly into these sidebar folders for advanced organization.",
  firstRunWebTourSortTitle: "Sort And Views",
  firstRunWebTourSortMsg:
    "Switch between favorites, recent items, visited bookmarks, alphabetical order, and domain grouping.",
  firstRunWebTourTagsTitle: "Tags",
  firstRunWebTourTagsMsg:
    "Filter by tags, search tags, clear selected tags, or open the full tags browser.",
  firstRunWebTourManageTitle: "Manage Tools",
  firstRunWebTourManageMsg:
    "Use these tools to rename folders, move selections, delete items, organize folders, run cleanup, and toggle selection.",
  firstRunWebTourCloudSyncTitle: "Cloud Backup & Sync",
  firstRunWebTourCloudSyncMsg:
    "Keep your bookmarks safe and synced across devices by connecting to Google Drive.",
  firstRunWebTourSmartCleanupTitle: "Smart Cleanup",
  firstRunWebTourSmartCleanupMsg:
    "Automatically organize domains, find dead links, remove duplicates, and clean up empty folders.",
  firstRunWebTourOrganizeTitle: "Folder Studio & Organizer",
  firstRunWebTourOrganizeMsg:
    "Open the advanced folder studio to reorganize, merge, and clean your folder tree.",
  firstRunWebTourOrganizeHubTitle: "Folder Studio Split-View",
  firstRunWebTourOrganizeHubMsg:
    "Visual 2-column workspace: Tree navigation on the left with Drag & Drop & Right-click menu, Inspector & Quick Actions on the right.",
  firstRunWebTourOrganizeModesTitle: "4 Powerful Studio Modes",
  firstRunWebTourOrganizeModesMsg:
    "Easily switch between: Hierarchy Studio (Tree), Merge Duplicates, A-Z Auto Sorting, and Clean Empty Folders.",
  firstRunWebTourAnalyticsTitle: "Analytics & Insights",
  firstRunWebTourAnalyticsMsg:
    "Track your bookmarking habits, access trends, and library health at a glance.",
  firstRunWebTourAnalyticsHubTitle: "Analytics Dashboard",
  firstRunWebTourAnalyticsHubMsg:
    "Comprehensive metrics: Total bookmarks, folders, tags, health ratio, and growth timeline charts.",
  firstRunWebTourAnalyticsTabsTitle: "Deep Analytics & Insights",
  firstRunWebTourAnalyticsTabsMsg:
    "Explore Activity trends, Domain & Tag distribution, Access habits, and Smart AI/cleanup suggestions.",
  firstRunWebTourSettingsMsg:
    "Open the right settings sidebar for language, theme, view mode, export/import, health checks, and app settings.",
  firstRunWebTourPanelMsg:
    "This right sidebar contains extension options, bookmark actions, quick-open behavior, health tools, and application settings.",
  firstRunTourSkip: "Skip",
  firstRunTourNext: "Next",
  firstRunTourBack: "Back",
  firstRunTourDone: "Done",
  fontSansSerif: "Aa: Sans-Serif (System Default)",
  fontRoboto: "Aa: Roboto (Google Font)",
  fontOpenSans: "Aa: Open Sans (Google Font)",
  fontInter: "Aa: Inter (Google Font)",
  fontLexend: "Aa: Lexend (Google Font)",
  fontGohu: "Aa: GohuFont (Pixel)",
  fontJetBrains: "Aa: JetBrains Mono",
  faviconMode: "Favicon Mode:",
  faviconSize: "Favicon Size:",
  faviconAuto: "Auto (Google → Hostname)",
  faviconGoogle: "Google Only",
  faviconHostname: "Hostname Only",
  headerLineConfig: "Header Line Style:",
  linePattern: "Slanted Pattern",
  lineSimple: "Simple Line",
  lineHidden: "Hidden",
  lineCustom: "Custom",
  headerLineColor: "Color",
  headerLineHeight: "Thickness",
  bookmarkMenuBgConfig: "Bookmark Menu BG:",
  bookmarkMenuBgGlass: "Glass like Tree View",
  bookmarkMenuBgNormal: "Normal solid",
  updateTitle: "Updated Successfully!",
  updateMessage: "Zero Bookmark Manager updated to ",
  back: "Back",
  openChat: "Open Chat",
  releaseNotes: "Release Notes",
  close: "Close",
  detailTitle: "Title",
  detailUrl: "URL",
  detailNotes: "Notes",
  commandPaletteTitle: "Command Palette",
  cloudSyncTestWarning:
    'Note: Google Drive Sync is currently in test mode (pending domain verification). If your login is blocked or restricted, please <a href="https://docs.google.com/forms/d/e/1FAIpQLSe0MVV6-Z7sxb-4Zpv5wRyBcKPsl8EbOqRAnntgrdBc_gOXpQ/viewform?usp=dialog" target="_blank" style="color: var(--accent-color); text-decoration: underline; font-weight: 600;">submit your email via this form &nearr;</a> to be granted access.',
  cloudSyncTitle: "Cloud Sync & Backup",
  cloudSyncSubtitle:
    "Backup and restore your bookmarks securely across devices.",
  cloudSyncProviderGithub: "GitHub",
  cloudSyncProviderGitlab: "GitLab",
  cloudSyncProviderGdrive: "Google Drive",
  cloudSyncBackupBtn: "Backup to Cloud",
  cloudSyncRestoreBtn: "Restore from Cloud",
  cloudSyncTokenLabel: "Personal Access Token (PAT):",
  cloudSyncTokenPlaceholder: "ghp_... or glpat-...",
  cloudSyncGistIdLabel: "Gist ID (optional):",
  cloudSyncGistIdPlaceholder: "Leave blank to auto-create new Secret Gist",
  cloudSyncGitlabHostLabel: "GitLab Instance URL:",
  cloudSyncGitlabSnippetLabel: "Snippet ID (optional):",
  cloudSyncGitlabSnippetPlaceholder: "Leave blank to auto-create new Snippet",
  cloudSyncGuideTitle: "Step-by-step Setup Guide",
  cloudSyncTokenSecureNotice:
    "Tokens are stored locally in your browser storage and never transmitted elsewhere.",
  supportTitle: "Support",
  donateTitle: "Support Project",
  donateDesc: "Help keep Zero Bookmark Manager free & updated!",
  donateModalTitle: "Support Developer",
  donateModalSubtitle:
    "Every cup of coffee fuels new features and improvements!",
  donateScanHint: "Scan with MoMo or any Banking app",
  donateBank: "Banking",
  donateScanBankHint: "Scan with any Banking app",
  donateKofiTitle: "Buy me a Coffee on Ko-fi",
  donateKofiDesc:
    "Support quickly and securely via International Card or PayPal.",
  donateOpenKofi: "Open Ko-fi Page",
  donateThankYou:
    "💖 Thank you for using and supporting Zero Bookmark Manager!",
  cpTabAll: "All",
  cpTabBookmarks: "Bookmarks",
  cpTabNotes: "Notes",
  cpTabFolders: "Folders",
  cpTabCommands: "Commands",
  snoozeBookmark: "Snooze",
  snoozeTitle: "Snooze",
  snooze1Hour: "In 1 hour",
  snoozeTomorrow: "Tomorrow",
  snoozeNextWeek: "Next week",
  snoozeSuccess: "Snoozed for {0} minutes!",
  analyticsTitle: "Analytics & Insights",
  saveSession: "Save Current Session (Tabs)",
  openInFullTab: "Open in Full Tab",
  analyticsDesc:
    "Your bookmarking habits, trends, and organization health at a glance.",
  analyticsTabOverview: "Overview",
  analyticsTabActivity: "Activity & Trends",
  analyticsTabDistribution: "Domains & Tags",
  analyticsTabHabits: "Access Habits",
  analyticsTabInsights: "Smart Insights",
  analyticsTotalBookmarks: "Total Bookmarks",
  analyticsTotalFolders: "Total Folders",
  analyticsAddedThisWeek: "Added This Week",
  analyticsAddedThisMonth: "Added This Month",
  analyticsTaggedRate: "Tagged",
  analyticsNotesRate: "With Notes",
  analyticsFavorites: "Favorites",
  analyticsPinned: "Pinned",
  analyticsSecurityRate: "HTTPS Secure",
  analyticsLinkHealth: "Link Health",
  analyticsHealthHealthy: "Healthy",
  analyticsHealthBroken: "Broken",
  analyticsHealthUnchecked: "Unchecked",
  analyticsMonthlyTimeline: "Monthly Additions (Past 12 Months)",
  analyticsDayOfWeek: "Peak Activity by Day of Week",
  analyticsTopDomains: "Top Domains",
  analyticsTopTags: "Top Used Tags",
  analyticsLargestFolders: "Largest Folders",
  analyticsMostVisited: "Most Visited Bookmarks",
  analyticsStaleBookmarks: "Forgotten / Stale Bookmarks (> 6 months)",
  analyticsInsightsTitle: "Smart Recommendations",
  analyticsInsightUntagged: "bookmarks have no tags assigned.",
  analyticsInsightUntaggedAction: "Suggest Tags",
  analyticsInsightUnsorted: "bookmarks are sitting in root without a folder.",
  analyticsInsightUnsortedAction: "Organize Folders",
  analyticsInsightBroken: "broken or slow links detected.",
  analyticsInsightBrokenAction: "Run Health Cleanup",
  analyticsInsightGreatJob:
    "Your bookmark library is in pristine condition! No optimization needed.",
  visitTrackingHint:
    "Click open bookmarks to record visit counts. Recent bookmarks shown below:",
  quickAccessItems: "Quick access items",
  items: "items",
  secureLinks: "secure links",
  analyticsTimeOfDay: "Time of Day Distribution",
  morning: "Morning",
  afternoon: "Afternoon",
  evening: "Evening",
  night: "Night",
  daySun: "Sun",
  dayMon: "Mon",
  dayTue: "Tue",
  dayWed: "Wed",
  dayThu: "Thu",
  dayFri: "Fri",
  daySat: "Sat",
  noDomainData: "No domain data yet",
  noTagsUsed: "No tags used yet",
  noFoldersFound: "No folders found",
  noForgottenBookmarks: "No forgotten bookmarks found!",
  savedOn: "Saved",
  visits: "visits",
  recent: "Recent",
  insightTagsDesc: "Organizing with tags makes searching instant and flexible.",
  insightFoldersDesc:
    "Group related bookmarks into subfolders for a tidy hierarchy.",
  insightHealthDesc: "Clean dead links to keep your library healthy and fast.",
  recentActivityTimeline: "Recently Added Bookmarks",
  scanNow: "Scan Now",
  scanHealthTitle: "Scan link health",
}
